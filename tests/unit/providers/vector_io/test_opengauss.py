...
class TestOpenGaussVectorIOAdapter:
    ...
    async def test_insert_and_query_chunks(self, opengauss_adapter, vector_db, sample_chunks):
        """Test the full adapter flow of inserting and querying."""
        await opengauss_adapter.register_vector_db(vector_db)
        await opengauss_adapter.insert_chunks(vector_db.identifier, sample_chunks)

        # Query for something semantically similar to the first chunk
        response = await opengauss_adapter.query_chunks(vector_db.identifier, "What color is the sky?")

        assert isinstance(response, QueryChunksResponse)
        assert len(response.chunks) > 0
        assert response.chunks[0].content == "The sky is blue."

    async def test_not_implemented_errors(self, opengauss_adapter):
        """Test that unsupported methods raise NotImplementedError."""
        with pytest.raises(NotImplementedError):
            await opengauss_adapter.openai_create_vector_store(name="test")
