...
async def test_authorized_fetch_with_where_sql_access_control(mock_get_authenticated_user):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        rows = await sqlstore.fetch_all("documents", policy=default_policy(), where={"id": 1})
        assert len(rows) == 1
        ...
        rows = await sqlstore.fetch_all("documents", policy=default_policy(), where={"id": 2})
        assert len(rows) == 1
...
async def test_authorized_store_user_attribute_capture(mock_get_authenticated_user):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        rows = await base_sqlstore.fetch_all("user_data", order_by=[("id", "asc")])
        assert len(rows) == 3

        # First item should have full attributes
        assert rows[0]["id"] == "item1"
        assert rows[0]["access_attributes"] == {"roles": ["editor"], "teams": ["content"], "projects": ["blog"]}

        # Second item should have null attributes (user with no attributes)
        assert rows[1]["id"] == "item2"
        assert rows[1]["access_attributes"] is None

        # Third item should have null attributes (no authenticated user)
        assert rows[2]["id"] == "item3"
        assert rows[2]["access_attributes"] is None
