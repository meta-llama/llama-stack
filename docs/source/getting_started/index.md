...
### i. Build the Llama Stack Config for Ollama
```bash
...
pprint("Non-streaming ...")
response = agent.create_turn(
    messages=[{"role": "user", "content": "Who are you?"}],
    session_id=s_id,
    stream=False,
)
pprint("agent>", response.output_message.content)

# Streamining with print helper
pprint("Streaming with print helper...")
...
```
Which will product some output similar to the output below:
```
Non-streaming ...
