...
from llama_stack.providers.utils.telemetry.tracing import get_current_span, span
...
class InferenceRouter(Inference):
    ...
    async def openai_completion(
        ...
        if stream:
            ...
            # return await self._accumulate_streaming_completion(
        ...
        if self.telemetry:
            async with span(
                "openai_chat_completion_metrics",
                {
    ...
    async def openai_chat_completion(
        ...
        if stream:
            ...
            async def stream_generator():
                async for chunk in self.stream_tokens_and_compute_metrics_openai_chat(
                    response=response_stream,
                    model=model_obj,
                    messages=messages,
                ):
            ...
            async for chunk in response:
                ...
                if chunk.choices and chunk.choices[0].finish_reason:
                    ...
                    if self.telemetry and chunk.usage:
                        ...
                        async with span(
                            "streaming_openai_chat_completion_metrics",
                            {
                                "model_id": model.model_id,
                                "provider_id": model.provider_id,
