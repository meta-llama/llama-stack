...
```
RamaLama requires the inference model to be the fully qualified path to the model on disk.
...
podman run \
  ...
  --env RAMALAMA_URL=http://host.containers.internal:8080
```
...
docker run \
  ...
  --port $LLAMA_STACK_PORT \
  --env INFERENCE_MODEL=$INFERENCE_MODEL \
  --env RAMALAMA_URL=http://host.docker.internal:8080
```
