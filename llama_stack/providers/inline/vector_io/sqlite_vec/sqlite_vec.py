...
class SQLiteVecIndex(EmbeddingIndex):
    ...
    async def query_hybrid(
        ...
        # Get chunks and scores in order
        chunks = []
        scores = []
        for doc_id, score in filtered_items:
            # Find chunk in either response by matching generated chunk_id
            chunk = None
            for c in vector_response.chunks:
                if generate_chunk_id(c.metadata["document_id"], str(c.content)) == doc_id:
                    chunk = c
                    break
            if chunk is None:
