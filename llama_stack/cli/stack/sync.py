...
class StackSync(Subcommand):
    def __init__(self, subparsers: argparse._SubParsersAction):
        super().__init__()
        self.parser = subparsers.add_parser(
            "sync",
            prog="llama stack sync",
            description="sync the dependencies for a llama stack distribution",
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        )
        self._add_arguments()
        self.parser.set_defaults(func=self._run_stack_sync_command)
...
the build. If not specified, currently active environment will be used if found.
    ...
    def _run_stack_sync_command(self, args: argparse.Namespace) -> None:
        # always keep implementation completely silo-ed away from CLI so CLI
        # can be fast to load and reduces dependencies
        from ._sync import run_stack_sync_command

        return run_stack_sync_command(args)
