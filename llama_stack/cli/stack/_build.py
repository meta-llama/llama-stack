...
def _generate_run_config(
    ...
    provider_registry = get_provider_registry(build_config, building=True)
    for api in apis:
        run_config.providers[api] = []
        providers = build_config.distribution_spec.providers[api]
        run_config.providers[api].extend(providers)
        ...
        for i, provider in enumerate(providers):
            ...
            p_spec = Provider(
                provider_id=f"{pid}-{i}" if len(provider.provider_type) > 1 else pid,
